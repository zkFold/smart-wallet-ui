...
module ZkFold.Cardano.SmartWallet.Server.Orphans () where
...
import ZkFold.Cardano.SmartWallet.Server.Auth (APIKeyAuthProtect, apiKeyHeaderText)
...
instance forall lang ftype api. (HasForeign lang ftype api, HasForeignType lang ftype Text) => HasForeign lang ftype (APIKeyAuthProtect :> api) where
